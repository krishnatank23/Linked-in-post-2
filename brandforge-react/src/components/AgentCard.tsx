...
function SectionHeader({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="mb-3">
      <div className="text-xs uppercase tracking-[0.2em] text-white/35 mb-1">{title}</div>
      {subtitle ? <div className="text-sm text-white/50 leading-6">{subtitle}</div> : null}
    </div>
  );
}
...
function DetailRow({ label, value }: { label: string; value: any }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="text-[10px] uppercase tracking-[0.18em] text-white/35 mb-2">{label}</div>
      <div className="text-sm text-white/80 leading-6 break-words">{fieldValue(value)}</div>
    </div>
  );
}
...
function renderList(items: any[], fallback: string) {
  if (!items || !items.length) {
    return <p className="text-sm text-white/45">{fallback}</p>;
  }

  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item, i) => (
        <span key={i} className="px-3 py-1 rounded-full bg-white/10 border border-white/10 text-[11px] font-semibold text-white/75">
...
const AgentCard = ({ data, index }: AgentCardProps) => {
  ...
  const renderContent = () => {
      ...
      return (
        <div className="space-y-5">
          <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
            ...
            <p className="text-sm text-white/75 leading-7">{profile.professional_summary || profile.summary || profile.overview || 'Resume parsed successfully.'}</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            {experience.length ? experience.map((item: any, i: number) => (
              <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-2">
                <div className="font-semibold text-white/90">{fieldValue(item.role)}{item.company ? ` at ${fieldValue(item.company)}` : ''}</div>
                <div className="text-xs text-white/45">{fieldValue(item.duration)}</div>
                <div className="text-sm text-white/70 leading-7">{fieldValue(item.description)}</div>
                {asArray(item.key_achievements).length ? (
                  <div className="space-y-2">
                    <div className="text-xs uppercase tracking-[0.18em] text-white/35">Key Achievements</div>
                    {renderList(asArray(item.key_achievements), 'No achievements listed.')}
                  </div>
                ) : null}
              </div>
            )) : <p className="text-sm text-white/45">No experience entries found.</p>}
          </div>
          ...
          <div className="space-y-3">
            ...
            {education.length ? education.map((item: any, i: number) => (
              <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
                <div className="font-semibold text-white/90">{fieldValue(item.institution)}</div>
                <div className="text-sm text-white/70">{fieldValue(item.degree)}{item.field_of_study ? `, ${fieldValue(item.field_of_study)}` : ''}</div>
                <div className="text-xs text-white/45">{fieldValue(item.year)}</div>
              </div>
            )) : <p className="text-sm text-white/45">No education entries found.</p>}
          </div>
          ...
          <div className="space-y-3">
            ...
            {projects.length ? projects.map((item: any, i: number) => (
              <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
                <div className="font-semibold text-white/90">{fieldValue(item.name)}</div>
                <div className="text-sm text-white/70 leading-7">{fieldValue(item.description)}</div>
      ...
      return (
        <div className="space-y-5">
          <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
            ...
            <p className="text-sm text-white/75 leading-7">{fieldValue(userPersona.professional_identity)}</p>
          </div>
          ...
          {output.industry_context_used ? (
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
              ...
              <p className="text-sm text-white/70 leading-7 whitespace-pre-wrap">{fieldValue(output.industry_context_used)}</p>
            </div>
          ) : null}
        </div>
      );
    ...
    if (agentName.includes('Influence') || agentName.includes('Idol Scout')) {
      ...
      return (
        <div className="space-y-3">
          <div className="p-4 rounded-2xl bg-accent/10 border border-accent/20 text-sm text-white/75">
            ...
            <div key={i} className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/10">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-white/45 truncate">{inf.snippet || inf.summary || 'LinkedIn benchmark candidate'}</div>
                </div>
              </div>
              {inf.link ? (
                <a href={inf.link} target="_blank" rel="noreferrer" className="text-white/35 hover:text-accent">
    ...
    if (agentName.includes('Posting Frequency Recommendation')) {
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
            <div className="text-xs uppercase tracking-[0.2em] text-white/35 mb-2">Cadence Rationale</div>
            <p className="text-sm text-white/70 leading-7">{output.rationale || 'Weekly cadence computed from gap depth and selected influencer count.'}</p>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="space-y-5">
          {perInfluencers.length > 0 ? (
            <div className="space-y-3">
              <div className="text-xs uppercase tracking-[0.2em] text-white/35">Selected Influencers</div>
              {perInfluencers.map((entry: any, idx: number) => (
                <div key={idx} className="p-4 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <div className="text-sm text-white/65 leading-7">{entry.analysis?.gap_analysis?.content_authority_gap || 'Gap analysis complete.'}</div>
                </div>
              ))}
            </div>
          ) : null}
          <div className="p-4 rounded-2xl bg-accent/10 border border-accent/20">
            ...
            <p className="text-sm text-white/75 leading-7">{overall.content_authority_gap || 'Combined gap summary generated.'}</p>
          </div>
          <div className="space-y-3">
            <div className="text-xs uppercase tracking-[0.2em] text-white/35">Content Pillars</div>
            {renderList(contentPillars, 'Content pillars will appear here.')}
          </div>
          {actionPlan.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs uppercase tracking-[0.2em] text-white/35">Action Plan</div>
              {actionPlan.map((step: string, idx: number) => (
                <div key={idx} className="flex items-start gap-3 p-3 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <div className="text-sm text-white/70 leading-7">{step}</div>
                </div>
              ))}
            </div>
          ) : null}
            ...
            {posts.slice(0, 3).map((post: any, idx: number) => (
              <div key={idx} className="p-4 rounded-2xl bg-white/5 border border-white/10">
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div className="font-semibold">{post.type || `Post ${idx + 1}`}</div>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(post.content || '');
                      toast.success('Post copied to clipboard');
                    }}
                    className="text-xs px-3 py-1 rounded-full bg-white/10 border border-white/10 text-white/70 hover:bg-white/20"
                  >
                    Copy
                  </button>
                </div>
                <p className="text-sm text-white/70 leading-7 whitespace-pre-wrap">{post.content || 'Drafting your post...'}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-2">
            <div className="text-xs uppercase tracking-[0.2em] text-white/35">Recipient</div>
            <div className="text-sm text-white/80 font-semibold break-words">{fieldValue(recipient)}</div>
          ...
          {output.reminder_message ? (
            <div className="p-4 rounded-2xl bg-accent/10 border border-accent/20">
              ...
              <p className="text-sm text-white/75 leading-7">{output.reminder_message}</p>
            </div>
          ) : null}
        </div>
      );
    ...
    return (
      <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
        <pre className="text-[11px] text-white/35 overflow-auto max-h-64 whitespace-pre-wrap">{JSON.stringify(output, null, 2)}</pre>
      </div>
    );
  };
  ...
  return (
    ...
    >
      <div className="p-6 md:p-7">
        <div className="flex items-start justify-between gap-4 mb-6">
          <div className="flex items-center gap-4 min-w-0">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isSuccess ? 'bg-primary/15 text-primary' : 'bg-white/5 text-white/25'}`}>
              <Cpu size={24} />
            </div>
            <div className="min-w-0">
              <div className="inline-flex items-center rounded-full border border-white/15 bg-white/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-white/65 mb-2">
                Agent {index + 1}
              </div>
              <div className="font-heading text-xl font-semibold truncate">{agentName.replace(' Agent', '')}</div>
              <div className="text-xs text-white/35">Autonomous graph output</div>
            </div>
          </div>
      ...
      <div className="mt-auto px-6 pb-6 pt-0 flex items-center justify-between border-t border-white/10">
        <div className="text-[10px] uppercase tracking-[0.24em] text-white/30 flex items-center gap-2">
          <Globe size={12} /> Live pipeline result
        </div>
        <button className="text-white/45 hover:text-accent transition-colors">
...
function StatCard({ label, value, icon: Icon }: { label: string; value: string; icon: any }) {
  return (
    <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
      ...
      <div className="text-sm text-white/80 font-semibold leading-6 break-words">{value}</div>
    </div>
  );
}
