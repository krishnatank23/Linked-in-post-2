  ...
  return (
    <div className={`${pageShell} px-4 py-6 md:px-8 lg:px-12`}>
      <div className={pageInner}>
        <header className={`${glassCard} p-5 md:p-6 mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between`}>
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl font-bold">BrandForge Studio Results</h1>
            </div>
            <p className="text-white/50">Full agent output history for {user?.username || 'your account'}.</p>
          </div>
        ...
        {loading ? (
          <div className={`${glassCard} p-10 text-center text-white/60`}>Loading results...</div>
        ) : results.length ? (
            ...
            {latestPostResult ? (
              <section className={`${glassCard} p-6 md:p-7`}>
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    <div className="text-xs uppercase tracking-[0.2em] text-white/35 mb-2">Post Delivery</div>
                    <h2 className="font-heading text-2xl font-semibold">Send this post to {user?.email || 'your registered email'}</h2>
                    <p className="text-white/55 mt-2 leading-7">
                ...
                {latestDeliveryResult ? (
                  <div className="mt-5 rounded-2xl border border-accent/20 bg-accent/10 p-4 text-sm text-white/75 flex items-start gap-3">
          ...
          <div className={`${glassCard} p-12 text-center text-white/50`}>
