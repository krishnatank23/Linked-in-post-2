          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-[#1c1a17]/50 text-sm">Review each output in order before moving to the next step.</p>
              </div>
            </div>
          </div>
        ...
        <section className="grid xl:grid-cols-[0.32fr_0.68fr] gap-6">
          <aside className={`${glassCard} p-5 md:p-6 h-fit`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-xs uppercase tracking-[0.2em] text-[#1c1a17]/35 mb-1">Step {activeStep + 1} of 3</div>
                <h2 className="font-heading text-2xl font-semibold">{STEP_CONFIG[activeStep].title}</h2>
              </div>
              <Clock3 size={18} className="text-[#1c1a17]/35" />
            </div>
            <p className="text-sm text-[#1c1a17]/55 leading-6 mb-5">{STEP_CONFIG[activeStep].description}</p>
              ...
              {STEP_CONFIG.map((config, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left rounded-2xl border p-4 transition-all ${isActive ? 'border-primary bg-primary/10' : completed ? 'border-accent/30 bg-black/5' : 'border-black/10 bg-black/5 hover:border-black/20'}`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-xs uppercase tracking-[0.18em] text-[#1c1a17]/35 mb-1">{index + 1}</div>
          ...
          <main>
            {loading ? (
              <div className={`${glassCard} p-10 text-center text-[#1c1a17]/60`}>Loading workflow results...</div>
            ) : currentResult ? (
              <div className="space-y-4">
                <div className={`${glassCard} p-5 md:p-6`}>
                  <div className="text-xs uppercase tracking-[0.2em] text-[#1c1a17]/35 mb-2">Current Output</div>
                ...
                {activeStep === 2 ? (
                  <>
                    <div className={`${glassCard} p-5 md:p-6`}>
                      <div className="text-xs uppercase tracking-[0.2em] text-[#1c1a17]/35 mb-2">Gap Analyzed</div>
                      {gapResult ? (
                        <div className="space-y-3">
                          <div className="rounded-2xl border border-primary/25 bg-primary/10 p-4">
                            <div className="text-sm font-semibold text-[#1c1a17]/90 mb-2">Overall Gap Summary</div>
                            <p className="text-sm text-[#1c1a17]/70 leading-7">
                              {String(
                          ...
                          {(gapResult.output?.overall_gap_analysis?.key_missing_elements || gapResult.output?.gap_analysis?.key_missing_elements || []).length ? (
                            <div className="rounded-2xl border border-black/10 bg-black/5 p-4">
                              <div className="text-xs uppercase tracking-[0.18em] text-[#1c1a17]/35 mb-3">Key Missing Elements</div>
                              <div className="flex flex-wrap gap-2">
                                {(gapResult.output?.overall_gap_analysis?.key_missing_elements || gapResult.output?.gap_analysis?.key_missing_elements || [])
                                  .slice(0, 8)
                                  .map((item: any, idx: number) => (
                                    <span key={`${item}-${idx}`} className="px-3 py-1 rounded-full bg-black/10 border border-black/10 text-[11px] font-semibold text-[#1c1a17]/75">
                                      {String(item)}
                                    </span>
                                  ))}
                              </div>
                    ...
                    <div className={`${glassCard} p-5 md:p-6`}>
                      <div className="text-xs uppercase tracking-[0.2em] text-[#1c1a17]/35 mb-2">All Agent Outputs</div>
                      <p className="text-[#1c1a17]/55 text-sm leading-7 mb-4">
              ...
              <div className={`${glassCard} p-10 text-center text-[#1c1a17]/60`}>
